!function(a, b, c) {
    ...
    return a.prototype.config = {
        ...
        if (c.length) c = b.loadImg(c);
            ...
            if (!(c instanceof HTMLImageElement)) throw new Error("IllegalInput: The input image is neither an URL string nor an image.");
        }
        d = d || {};
        var e = this.config,
            f = d.t || e.t,
        ...
        for (C = 0; 4 * (C + g) <= y.length && C + g <= B.length; C += g) {
            ...
            for (v = 4 * C; 4 * (C + G.length) > v && v < y.length; v += 4) y[v + 3] = G[v / 4 % g];
            E = G.length
        }
        ...
        if (a.length) a = b.loadImg(a);
            ...
            if (!(a instanceof HTMLImageElement)) throw new Error("IllegalInput: The input image is neither an URL string nor an image.");
        }
        c = c || {};
        var d = this.config,
            e = c.t || d.t,
            ...
            i = c.messageCompleted || d.messageCompleted;
